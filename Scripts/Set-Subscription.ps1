function Set-Subscription {
    ...
    If ([string]::IsNullOrEmpty($subscriptionId)) {
        ...
        Write-Host ('Your subscription is {0}' -f (Get-AzContext).Subscription.Name) -ForegroundColor Cyan
        ...
        If ($prompt.ToUpper() -eq 'Y' -or $prompt.ToUpper() -eq 'YES'){
            ...
            if ($null -eq $subscription){
                ...
                Write-Host ('Exiting...subscription still {0}' -f (Get-AzContext).Subscription.Name) -ForegroundColor Cyan
                Break

            }
            
            $Sub = Set-AzContext -Tenant (Get-AzContext).Tenant.Id -SubscriptionId $Subscription.SubscriptionId
            Write-Host ('Setting context to: {0}' -f (Get-AzSubscription -SubscriptionId $Sub.Subscription).Name) -ForegroundColor Cyan
