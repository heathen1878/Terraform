locals {
    ...
    virtual_machine_output = {
        for virtual_machine_key, virtual_machine_value in local.virtual_machine : virtual_machine_key => {
            ...
            public_ip_address          = try(local.virtual_machine[virtual_machine_key].public_ip_address, false)
            ...
            computer_name              = virtual_machine_value.operating_system == "linux" ? format("%s-%s", azurecaf_name.linux_virtual_machine[virtual_machine_key].result, substr(virtual_machine_key, 2, -1)) : format("%s-%s", azurecaf_name.windows_virtual_machine[virtual_machine_key].result, substr(virtual_machine_key, 2, -1))
            admin_username             = virtual_machine_value.operating_system == "linux" ? format("%s-adm", local.linux_virtual_machine[virtual_machine_key].name) : format("%s-adm", local.windows_virtual_machine[virtual_machine_key].name)
